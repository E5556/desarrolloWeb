...
<html lang="en">
	<head>
	    ...
	    <title><?php echo $_seo_scat ? htmlspecialchars($_seo_scat['subcategory']).' | '.$_SITE_NAME : 'Subcategoría | '.$_SITE_NAME; ?></title>
...
<!-- ============================================== HEADER : END ============================================== -->
<?php
$_bc = ($__r = mysqli_query($con,
    "SELECT s.subcategory, s.categoryid, c.categoryName
     FROM subcategory s JOIN category c ON c.id = s.categoryid
     WHERE s.id = $cid")) ? mysqli_fetch_assoc($__r) : null;
?>
    ...
    <div class="container">
        <div class="breadcrumb-inner">
            <ul class="list-inline list-unstyled">
                ...
                <li><a href="category.php?cid=<?php echo intval($_bc['categoryid'] ?? 0); ?>"><?php echo htmlspecialchars($_bc['categoryName'] ?? 'Categoría'); ?></a></li>
                <li class="active"><?php echo htmlspecialchars($_bc['subcategory'] ?? 'SubCategoría'); ?></li>
            </ul>
        </div>
    </div>
</div>
...
<?php
// Obtener info de esta subcategoría
$_scinfo = ($__r = mysqli_query($con,"SELECT categoryid FROM subcategory WHERE id=$cid")) ? mysqli_fetch_assoc($__r) : null;
$_parent_cid = intval($_scinfo['categoryid'] ?? 0);
$_parent_cat = ($__r = mysqli_query($con,"SELECT id,categoryName FROM category WHERE id=$_parent_cid")) ? mysqli_fetch_assoc($__r) : null;

// Subcategorías hermanas
$_subs_res = mysqli_query($con,"SELECT id,subcategory FROM subcategory WHERE categoryid=$_parent_cid");
$_subs = [];
while($_sr = mysqli_fetch_assoc($_subs_res)) $_subs[] = $_sr;
?>

<?php if(!empty($_subs)): ?>
<div class="ps-sidebar-group">
    <div class="ps-sidebar-toggle" onclick="this.parentElement.classList.toggle('open')">
        <span><span class="ps-icon"><i class="fa fa-list"></i></span> &nbsp;<?php echo htmlspecialchars($_parent_cat['categoryName'] ?? 'Subcategorías'); ?></span>
        <span class="ps-arrow">▼</span>
    </div>
    <div class="ps-sidebar-body">
        <?php foreach($_subs as $_s): ?>
        <a href="sub-category.php?scid=<?php echo (int)$_s['id']; ?>"<?php echo ($_s['id']==$cid)?' style="color:#c0396b;font-weight:600"':''; ?>>
            <span class="dot"></span><?php echo htmlspecialchars($_s['subcategory']); ?>
        </a>
        <?php endforeach; ?>
        <a href="category.php?cid=<?php echo $_parent_cid; ?>" class="see-all">
...
<div class="ps-sidebar-group">
    ...
    <div class="ps-sidebar-body">
        ...
        <a href="category.php?cid=<?php echo $row['id']; ?>"<?php echo ($row['id']==$_parent_cid)?' style="color:#c0396b;font-weight:600"':''; ?>>
...
<div class="ps-sidebar-group open">
    ...
    <div class="ps-sidebar-body" style="padding:12px 16px">
        ...
        <?php
        $_pr = mysqli_query($con,"SELECT MIN(productPrice) as mn, MAX(productPrice) as mx FROM products p WHERE p.subCategory='$cid'");
        ...
        <div class="ps-price-range">
            <span id="scat-price-label-min"><?php echo '$'.number_format($_cur_min,0,',','.'); ?></span>
            <span id="scat-price-label-max"><?php echo '$'.number_format($_cur_max,0,',','.'); ?></span>
        </div>
        <input id="scat-price-slider" type="text" style="display:none"
        ...
        <div class="ps-price-inputs">
            <input type="number" id="scat-pmin-input" value="<?php echo $_cur_min; ?>" min="<?php echo $_mn; ?>" max="<?php echo $_mx; ?>">
            <span>—</span>
            <input type="number" id="scat-pmax-input" value="<?php echo $_cur_max; ?>" min="<?php echo $_mn; ?>" max="<?php echo $_mx; ?>">
        </div>
        <a id="scat-price-apply" href="<?php echo htmlspecialchars(_scat_url(['pmin'=>$_cur_min,'pmax'=>$_cur_max,'pag'=>1])); ?>" class="btn btn-primary btn-sm ps-price-apply">Aplicar filtro</a>
        <?php if($_pmin > 0 || $_pmax > 0): ?>
        <a href="<?php echo htmlspecialchars(_scat_url(['pmin'=>'','pmax'=>'','pag'=>1])); ?>" style="display:block;text-align:center;font-size:11px;color:#c0396b;margin-top:5px">Limpiar filtro &times;</a>
        <?php endif; ?>
    </div>
</div>
<script>
$(document).ready(function(){
    if(!$('#scat-price-slider').length) return;
    try {
        $('#scat-price-slider').slider({tooltip:'hide'}).on('slide', function(e){
            var v = e.value;
            $('#scat-pmin-input').val(v[0]);
            $('#scat-pmax-input').val(v[1]);
            $('#scat-price-label-min').text('$'+v[0].toLocaleString('es-CO'));
            $('#scat-price-label-max').text('$'+v[1].toLocaleString('es-CO'));
            updateScatPriceLink(v[0], v[1]);
        });
    } catch(e){}
    $('#scat-pmin-input, #scat-pmax-input').on('change', function(){
        updateScatPriceLink($('#scat-pmin-input').val(), $('#scat-pmax-input').val());
    });
    function updateScatPriceLink(mn, mx){
        var base = '<?php echo htmlspecialchars(_scat_url(['pmin'=>'__MN__','pmax'=>'__MX__','pag'=>1])); ?>';
        base = base.replace('__MN__', mn).replace('__MX__', mx);
        $('#scat-price-apply').attr('href', base);
    }
});
</script>
	...
	<?php
	$sub_row = ($__r = mysqli_query($con, "SELECT subcategory, subcategoryImage FROM subcategory WHERE id=$cid")) ? mysqli_fetch_assoc($__r) : null;
	$sub_banner = (!empty($sub_row['subcategoryImage']))
		? 'admin/subcategoryimages/' . htmlspecialchars($sub_row['subcategoryImage'])
		: 'assets/images/banners/cat-banner-2.jpg';
	?>
	<style>
	...
	.ps-cat-label {
		position:absolute; top:50%; left:6%; z-index:2; color:#fff;
	...
	<div class="ps-cat-banner hidden-xs">
		<img src="<?php echo $sub_banner; ?>" alt="<?php echo htmlspecialchars($sub_row['subcategory'] ?? ''); ?>">
		<div class="ps-cat-label">
			<span class="ps-cat-tag">SubCategoría</span>
			<?php if (!empty($_bc['categoryName'])): ?>
			<span class="ps-cat-parent"><?php echo htmlspecialchars($_bc['categoryName']); ?></span>
			<?php endif; ?>
			<h2><?php echo htmlspecialchars($sub_row['subcategory'] ?? ''); ?></h2>
...
if ($_sort === 'vendidos') {
    $_count_q = mysqli_query($con, "SELECT COUNT(*) as total FROM products WHERE subCategory='$cid'");
    $_total = ($r = mysqli_fetch_assoc($_count_q)) ? (int)$r['total'] : 0;
    $_ret = mysqli_query($con, "SELECT p.*, COALESCE(SUM(o.quantity),0) as total_sold FROM products p LEFT JOIN orders o ON o.productId=p.id AND o.paymentMethod IS NOT NULL WHERE p.subCategory='$cid' GROUP BY p.id ORDER BY $_order_sql LIMIT $_per_page OFFSET $_offset");
} else {
    $_count_q = mysqli_query($con, "SELECT COUNT(*) as total FROM products WHERE subCategory='$cid'");
    $_total = ($r = mysqli_fetch_assoc($_count_q)) ? (int)$r['total'] : 0;
    $_ret = mysqli_query($con, "SELECT * FROM products p WHERE p.subCategory='$cid'$_price_cond ORDER BY $_order_sql LIMIT $_per_page OFFSET $_offset");
}
$_total_pages = max(1, ceil($_total / $_per_page));

function _scat_url($params) {
    $base = array_merge($_GET, $params);
    return 'sub-category.php?' . http_build_query($base);
}
?>
<style>
...
<div class="search-result-container">
    <div class="ps-toolbar">
        ...
        <button class="ps-view-btn active" id="btn-view-grid" title="Vista cuadrícula" onclick="psSetView('grid')"><i class="fa fa-th"></i></button>
        <button class="ps-view-btn" id="btn-view-list" title="Vista lista" onclick="psSetView('list')"><i class="fa fa-list"></i></button>
        </div>
        <select class="ps-sort-select" onchange="location.href=this.value">
            ...
            <option value="<?php echo htmlspecialchars(_scat_url(['sort'=>$_k,'pag'=>1])); ?>"<?php echo $_sort===$_k?' selected':''; ?>><?php echo $_v; ?></option>
            <?php endforeach; ?>
        </select>
    </div>
...
<?php endif; ?>
                ...
                <div class="ps-pagination">
                    <?php if($_page>1): ?><a href="<?php echo htmlspecialchars(_scat_url(['pag'=>$_page-1])); ?>">&#8592;</a><?php endif; ?>
                    <?php for($i=1;$i<=$_total_pages;$i++):
                        if($i==1||$i==$_total_pages||abs($i-$_page)<=1): ?>
                        <?php if($i==$_page): ?><span class="active"><?php echo $i; ?></span>
                        <?php else: ?><a href="<?php echo htmlspecialchars(_scat_url(['pag'=>$i])); ?>"><?php echo $i; ?></a><?php endif; ?>
                    <?php elseif(abs($i-$_page)==2): ?><span class="dots">&hellip;</span>
                    <?php endif; endfor; ?>
                    <?php if($_page<$_total_pages): ?><a href="<?php echo htmlspecialchars(_scat_url(['pag'=>$_page+1])); ?>">&#8594;</a><?php endif; ?>
                </div>
                <?php endif; ?>
...
function psSetView(mode) {
    ...
    if (!gc) return;
    if (mode === 'list') { gc.classList.add('list-view'); bg.classList.remove('active'); bl.classList.add('active'); }
    else { gc.classList.remove('list-view'); bl.classList.remove('active'); bg.classList.add('active'); }
