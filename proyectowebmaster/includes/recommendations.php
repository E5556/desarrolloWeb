...
function ps_recommendations($con, int $uid, int $limit = 6): array {
    ...
    $sql = "SELECT p.id, p.productName, p.productPrice, p.productImage, p.catId,
                   (CASE WHEN p.catId IN ($cat_in) THEN 1 ELSE 0 END) as prio
            FROM products p
