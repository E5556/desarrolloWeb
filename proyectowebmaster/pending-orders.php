...
header('location:login.php');
	...
	if (isset($_GET['id']) && isset($_GET['tok']) && hash_equals(csrf_token(), $_GET['tok'])) {
		$del_uid = intval($_SESSION['id']);
		$del_oid = intval($_GET['id']);
		mysqli_query($con, "DELETE FROM orders WHERE userId=$del_uid AND paymentMethod IS NULL AND id=$del_oid");
...
<html lang="en">
	<head>
	    ...
	    <title>Historial de pedidos pendientes | <?php echo $_SITE_NAME; ?></title>
		...
		<table class="table table-bordered">
			<thead>
				<tr>
					...
					<th class="cart-description item">Imágen</th>
...
?>
				<tr>
					...
					<td><a href="pending-orders.php?id=<?php echo intval($row['oid']); ?>&tok=<?php echo htmlspecialchars(csrf_token()); ?>" onclick="return confirm('¿Cancelar esta orden?')">Borrar</a></td>
				</tr>
<?php $cnt=$cnt+1;} ?>
<tr>
